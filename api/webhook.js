  ...
  try {
    ...
    const body = {
      ...
      interactive: {
        ...
        body: {
          text: "🔒 Escolha a vaga que deseja encerrar:",
        },
        footer: {
    ...
    try {
      ...
      if (!resp.ok) {
        console.error(
          "Erro ao enviar opções de encerrar vaga:",
          resp.status,
          txt
        );
      } else {
        console.log(
          "Opções de encerrar enviadas:",
          txt.slice(0, 500)
        );
      }
    } catch (e) {
    ...
    console.error("Erro geral em handleStartClose:", e);
    return res.status(500).send("erro interno");
  }
}
